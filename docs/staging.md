...
Phase 3 remains unaccepted until its canonical report records exact-HEAD CI, regenerated Supabase types and hosted evidence. Apply only the three additive repository migrations after local/CI verification, deploy a genuine protected Preview from the Phase 3 branch and rotate the exact Auth allowlist using the established procedure. Run the Phase 3, Phase 2, intake and foundation suites sequentially. Finish with scoped data/file/session cleanup and automation-bypass revocation. No Phase 3 cloud change has been made at the current progress checkpoint; the accepted Phase 2 Preview is not Phase 3 evidence.
