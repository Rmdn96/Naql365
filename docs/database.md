...
## Phase 3.5 migration and configuration

The accepted 13-migration foundation is extended by six migrations (19 total). Existing commercial data is backfilled to SA while retaining its previous values/timestamps. A populated upgrade test projects and compares every original commercial field and trigger state. New market/region/city/service/tax tables use relational keys, explicit constraints and RLS. Superseded foreign keys are removed only where stronger composite market relationships replace them; application embeddings use the resulting authoritative relationship metadata.

Tax versions are effective-dated and immutable in their commercial facts. Evaluations, sent quote versions and accepted orders preserve applied tax/currency. Historical pre-market tax snapshots remain valid and unchanged. Authoritative types are generated by Supabase in CI and against Staging; the hosted PostgREST version header is recorded separately from business schema differences.
