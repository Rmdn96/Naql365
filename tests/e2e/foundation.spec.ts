...
test('root defaults to Arabic and language switch reaches English',async({page})=>{
  await page.goto('/');await expect(page).toHaveURL(/\/ar$/);
...
for(const locale of ['ar','en']){
 test(`${locale} public page is accessible and has correct SEO`,async({page})=>{
  ...
  await expect(page.locator('link[rel="canonical"]')).toHaveAttribute('href',`http://127.0.0.1:3000/${locale}`);
 ...
 test(`${locale} showcase and dialog keyboard behavior`,async({page})=>{
  ...
  const trigger=page.getByRole('button',{name:locale==='ar'?'عرض الحوار':'Open dialog'});
...
test('protected spaces fail closed without configured credentials',async({page})=>{
 for(const route of ['account','portal','driver']){await page.goto(`/en/${route}`);await expect(page.getByRole('heading',{name:'Service not configured yet'})).toBeVisible();await expect(page.locator('meta[name="robots"]')).toHaveAttribute('content',/noindex/);}
});
test('unknown locale is not accepted',async({page})=>{const response=await page.goto('/fr');expect(response?.status()).toBe(404);});
test('callback rejects external destinations when no valid code is present',async({request})=>{
