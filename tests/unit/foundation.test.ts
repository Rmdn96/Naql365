...
describe('server authorization service', () => {
  it('rejects anonymous callers without consulting the adapter', async () => {
    ...
    await expect(authorize({ hasPermission }, null, 'portal.access')).rejects.toMatchObject({
  ...
  it('fails closed for denied permission', async () => {
    await expect(
      authorize({ hasPermission: async () => false }, principal, 'finance.read'),
  ...
  it('rejects malformed tenant context', async () => {
    await expect(
      authorize(
        ...
        { ...principal, organizationId: 'invalid' },
        'portal.access',
      ),
...
describe('input and redirect validation', () => {
  it.each([
    ...
    '//evil.example',
    '/\\evil.example',
    '/ar/%2f%2fevil.example',
    '/ar/account?next=evil',
    '/ar/account\r\nLocation: evil',
    'javascript:alert(1)',
    null,
  ...
  it('accepts only supported internal destinations', () =>
    expect(safeRedirect('/en/portal', 'ar')).toBe('/en/portal'));
  it('rejects unexpected authority fields', () =>
    expect(tenantInput.safeParse({ ...principal, role: 'SUPER_ADMIN' }).success).toBe(false));
  it('rejects file paths in place of stable identifiers', () =>
    expect(fileAccessInput.safeParse({ fileId: '../secret' }).success).toBe(false));
  it('never exposes internal exception messages', () =>
    expect(publicError(new Error('sensitive detail'))).toEqual({ code: 'internal' }));
  it('preserves safe typed error codes', () =>
    expect(publicError(new AppError('forbidden', 'private detail'))).toEqual({
      code: 'forbidden',
    }));
});
describe('locale foundations', () => {
  it('uses exact locale identifiers', () => {
    expect(isLocale('ar')).toBe(true);
    expect(isLocale('AR')).toBe(false);
    expect(isLocale('fr')).toBe(false);
  });
  it('supports both directions and scoped routes', () => {
    expect(direction('ar')).toBe('rtl');
    expect(direction('en')).toBe('ltr');
    expect(localizedPath('ar', '/account')).toBe('/ar/account');
  });
...
describe('environment boundaries', () => {
  it('fails closed for absent Supabase configuration', () => {
    vi.stubEnv('NEXT_PUBLIC_SUPABASE_URL', '');
    vi.stubEnv('NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY', '');
    expect(getPublicEnv()).toBeNull();
  });
  it('rejects insecure non-local Supabase origins', () => {
    vi.stubEnv('NEXT_PUBLIC_SUPABASE_URL', 'http://example.com');
    vi.stubEnv('NEXT_PUBLIC_SUPABASE_PUBLISHABLE_KEY', 'test-only');
    expect(getPublicEnv()).toBeNull();
  });
  it('requires a production canonical origin', () => {
    vi.stubEnv('NODE_ENV', 'production');
    vi.stubEnv('APP_URL', '');
    expect(() => appUrl()).toThrow();
  });
  it.each(['https://example.com/path', 'https://user:pass@example.com', 'http://example.com'])(
    'rejects unsafe canonical origin %s',
    (origin) => {
