...
it('enforces Phase 3 multi-trip execution, assignments, POD and operational isolation', async () => {
  const db = await foundationDatabase();
  try {
    await db.exec(
      readFileSync(new URL('../../supabase/tests/phase3.test.sql', import.meta.url), 'utf8').split(
        '-- Supabase TAP report',
      )[0]!,
    );
    const { rows } = await db.query<{ count: number }>(
      'select count(*)::int as count from public.trips',
    );
