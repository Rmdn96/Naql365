...
export type DriverJourneyHook = {
  ...
  tripId: string;
  orderId: string;
  driver: Awaited<ReturnType<typeof principal>>;
  replacement: Awaited<ReturnType<typeof principal>>;
  customer: Awaited<ReturnType<typeof principal>>;
};
export function registerDriverJourneys(
  hooks: {
    started?: (args: DriverJourneyHook) => Promise<void>;
    reassigned?: (args: DriverJourneyHook) => Promise<void>;
    completed?: (args: DriverJourneyHook) => Promise<void>;
  } = {},
) {
  for (const country of ['SA', 'EG'] as const)
    test(`hosted ${country} authenticated Driver execution, reassignment, issues, private POD and tracking`, async ({
      ...
      const driverRole = (await admin.from('roles').select('id').eq('code', 'DRIVER').single())
        .data!.id;
      expect(
        (
          await admin
      ...
      const hookArgs: DriverJourneyHook = {
        page,
        context,
        baseURL: baseURL!,
        country,
      ...
      await page.goto(`/${locale}/driver/trips/${trip1}`);
      await hooks.reassigned?.(hookArgs);
      await axe(page);
      ...
      await expect
        ...
        .toBe(1);
      const issue = (await admin.from('issues').select('id').eq('trip_id', trip1).single()).data!
        .id;
      ...
      await expect
        .poll(
          async () =>
            (await admin.from('trip_pods').select('state').eq('trip_id', trip1).single()).data
              ?.state,
        )
        .toBe('FINAL');
      await page.reload();
      ...
      expect(
        (await replacement.rpc('driver_finalize_pod', { p_trip: trip1, p_file: randomUUID() }))
          .error,
      ).not.toBeNull();
      // Allow the real network round trip, then verify expiry against the same issued URL.
      const signed = await replacement.storage
        .from('pod-files')
        .createSignedUrl(pod.object_name, 10);
      ...
      await hooks.completed?.(hookArgs);
      expect(
      ...
      await expect
        .poll(
          async () =>
            (await admin.from('trip_pods').select('state').eq('trip_id', trip2).single()).data
              ?.state,
        )
        .toBe('FINAL');
      await page.reload();
