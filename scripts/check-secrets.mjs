...
for (const file of files) {
  if (/(^|\/)\.env(?:\.|$)/.test(file) && file !== '.env.example') findings.push(`${file}: tracked environment file`);
  if (!existsSync(file) || /\.(png|jpg|woff2?)$/.test(file)) continue;
  const content=readFileSync(file,'utf8');
  if (patterns.some(pattern=>pattern.test(content))) findings.push(`${file}: possible credential (value redacted)`);
}
