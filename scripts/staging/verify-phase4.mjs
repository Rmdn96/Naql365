...
if (
  ...
  throw new Error('Verified protected Phase 4 Preview required');
...
try {
  ...
  query(
    ref,
    `insert into public.organizations(id,name) values('${otherOrg}','Phase 4 isolation fixture');select private.provision_initial_market_catalogue('${otherOrg}');update public.markets set active=true where organization_id='${otherOrg}'`,
  );
  const identities = {};
  for (const label of [
    'customer',
  ...
  console.error('Phase 4 hosted acceptance failed; sensitive details withheld');
  process.exitCode = 1;
} finally {
  try {
    ...
    console.log('Phase 4 synthetic fixture cleanup PASS; existing catalogues retained');
  } catch {
    console.error('Phase 4 cleanup incomplete; scoped identifiers withheld');
    process.exitCode = 1;
  } finally {
    release();
  }
