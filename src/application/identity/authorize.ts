...
export async function authorize(
  ...
  if (
    !entityId.safeParse(principal.organizationId).success ||
    !entityId.safeParse(principal.userId).success
  )
    throw new AppError('validation', 'Invalid principal');
  if (!(await port.hasPermission(principal, permission)))
    throw new AppError('forbidden', 'Permission required');
  return principal;
}
