...
export async function proxy(request: NextRequest) {
  ...
  const csp = ["default-src 'self'", `script-src 'self' 'nonce-${nonce}' 'strict-dynamic'${process.env.NODE_ENV === 'development' ? " 'unsafe-eval'" : ''}`,
    "style-src 'self' 'unsafe-inline'", "img-src 'self' data: blob:", "font-src 'self'", `connect-src 'self'${env ? ` ${new URL(env.url).origin}` : ''}${process.env.NODE_ENV === 'development' ? ' ws: wss:' : ''}`,
    "object-src 'none'", "base-uri 'self'", "form-action 'self'", "frame-ancestors 'none'"].join('; ');
  const headers = new Headers(request.headers);
  headers.set('x-nonce', nonce);
  headers.set('Content-Security-Policy', csp);
  let response = NextResponse.next({ request: { headers } });
  const protectedRoute = /^\/(ar|en)\/(account|portal|driver|login)(\/|$)/.test(request.nextUrl.pathname);
  if (env && protectedRoute) {
    const client = createServerClient(env.url, env.publishableKey, { cookies: {
